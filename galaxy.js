...
/*! Galaxy (OGL) - robust loader
 * - If window.OGL is missing, load it from same-origin (./ogl.umd.js) or window.OGL_SRC
 * - Then start GalaxyOGL with window.GALAXY_OGL_OPTIONS or window.GALAXY_OPTIONS
 */
(function (global) {
  'use strict';

  var OGL_SRC = global.OGL_SRC || 'ogl.umd.js'; // allow override: window.OGL_SRC = 'assets/ogl.umd.js'

  function ensureOGL(cb) {
    if (global.OGL) return cb();
    var s = document.createElement('script');
    s.src = OGL_SRC;
    s.onload = cb;
    s.onerror = function () {
      console.error('Failed to load OGL from', OGL_SRC);
    };
    document.head.appendChild(s);
  }

  var vertex = `attribute vec2 uv;
...
vec3 StarLayer(vec2 uv){ vec3 col=vec3(0.0); vec2 gv=fract(uv)-0.5; vec2 id=floor(uv);
  for(int y=-1;y<=1;y++){ for(int x=-1;x<=1;x++){ vec2 offset=vec2(float(x),float(y)); vec2 si=id+offset; float seed=Hash21(si); float size=fract(seed*345.32); float glossLocal=tri(uStarSpeed/(PERIOD*seed+1.0)); float flareSize=smoothstep(0.9,1.0,size)*glossLocal;
    ...
    float sat=length(base-vec3(dot(base,vec3(0.299,0.587,0.114))))*uSaturation; float val=max(max(base.r,base.g),base.b); base=hsv2rgb(vec3(hue,sat,val));
    vec2 pad=vec2(tris(seed*34.0 + uTime*uSpeed/10.0), tris(seed*38.0 + uTime*uSpeed/30.0)) - 0.5;
    float star=Star(gv - offset - pad, flareSize); float twinkle = trisn(uTime*uSpeed + seed*6.2831)*0.5 + 1.0; twinkle = mix(1.0, twinkle, uTwinkleIntensity); star *= twinkle;
...
void main(){ vec2 focalPx=uFocal*uResolution.xy; vec2 uv=(vUv*uResolution.xy - focalPx)/uResolution.y; vec2 mouseNorm=uMouse-vec2(0.5);
  ...
  } else if(uMouseRepulsion){ vec2 mousePosUV=(uMouse*uResolution.xy - focalPx)/uResolution.y; float mouseDist=length(uv-mousePosUV); vec2 repulsion=normalize(uv-mousePosUV)*(uRepulsionStrength/(mouseDist+0.1)); uv += repulsion*0.05;
  } else { vec2 mouseOffset = mouseNorm*0.1; uv += mouseOffset; }
  ...
  function start() {
    var OGL = global.OGL;
    if (!OGL) { console.error('OGL still missing'); return; }

    var renderer = new OGL.Renderer({ alpha: DEFAULTS.transparent, premultipliedAlpha: false, dpr: Math.min(2, global.devicePixelRatio||1) });
    var gl = renderer.gl;
    var cnv = gl.canvas;
    cnv.id = 'galaxy_ogl';
    ...
    if (DEFAULTS.transparent) { gl.clearColor(0,0,0,0); gl.enable(gl.BLEND); gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA); } else { gl.clearColor(0,0,0,1); }
    ...
    var program = new OGL.Program(gl, {
      ...
      uniforms: {
        ...
        uFocal: { value: new Float32Array(DEFAULTS.focal) },
        uRotation: { value: new Float32Array(DEFAULTS.rotation) },
        uStarSpeed: { value: DEFAULTS.starSpeed },
        ...
        uMouseRepulsion: { value: DEFAULTS.mouseRepulsion },
        uTwinkleIntensity: { value: DEFAULTS.twinkleIntensity },
        uRotationSpeed: { value: DEFAULTS.rotationSpeed },
        uRepulsionStrength: { value: DEFAULTS.repulsionStrength },
        uMouseActiveFactor: { value: 0.0 },
        uAutoCenterRepulsion: { value: DEFAULTS.autoCenterRepulsion },
        uTransparent: { value: DEFAULTS.transparent }
      }
    });
    ...
    function loop(t){
      ...
      program.uniforms.uStarSpeed.value = (t*0.001*DEFAULTS.starSpeed)/10.0;
      var k=0.05;
