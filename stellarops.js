...
*/
'use strict';
window.stellarops = new (function() {
...
if(false && current.dev) console.log("setCurrentSlot(): slot "+slotgroup+ " #"+slotnum+" is "+(reserved ? ("reserved: "+Object.keys(reserved).join(",")) : "unreserved"));
	...
	*/
	...
	// Custom: freeze version check and show our own label
	try {
		var vlabel = document.getElementById('version_label');
		if (vlabel) {
			// remove any dynamic translation hooks so our text isn't replaced
			vlabel.removeAttribute('edsy-vals');
			vlabel.removeAttribute('edsy-text');
			vlabel.removeAttribute('edsy-title');
			vlabel.innerHTML = 'v 1.0';
			vlabel.title = '';
		}
	} catch (e) { /* ignore */ }
